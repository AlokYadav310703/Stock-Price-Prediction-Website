  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <Panel
          title="Stage 1 · Base Learners"
          subtitle="30-day OHLC window ensemble"
        >
          <div className="grid grid-cols-2 gap-4">
            <SubMetric
              label="LSTM Model"
              value={fmtUSD(p.base_predictions.lstm)}
              delta={p.base_predictions.lstm - p.current_price}
            />
            <SubMetric
              label="CNN Model"
              value={fmtUSD(p.base_predictions.cnn)}
              delta={p.base_predictions.cnn - p.current_price}
            />
          </div>
        </Panel>

        <Panel
          title="Stage 2 · News-Aware Correction"
          subtitle="Gradient-boosted adjustment"
        >
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs text-paper-dim mb-1">Adjustment</div>
              <div
                className={`num text-xl font-semibold ${p.correction >= 0 ? "text-signal-up" : "text-signal-down"}`}
              >
                {p.correction >= 0 ? "+" : ""}
                {fmtUSD(p.correction)}
              </div>
        ...
        <Panel
          title="News Sentiment Inputs"
          subtitle={`${p.news_features.news_count} articles analyzed`}
        >
          <div className="grid grid-cols-3 gap-4">
            <SubMetric
              label="Sentiment"
              value={p.news_features.sentiment_score}
            />
            <SubMetric
              label="Impact Score"
              value={p.news_features.impact_score}
            />
            <SubMetric
              label="Event Weight"
              value={p.news_features.event_weight}
            />
          </div>
          {p.news_features.has_supply_chain_event ? (
            <div className="mt-4 text-xs text-signal-warn">
              ⚠ Supply chain event detected in recent coverage
            </div>
          ) : (
            <div className="mt-4 text-xs text-signal-up">
              ✓ No supply chain issues flagged
            </div>
          )}
        </Panel>
        ...
        <Panel title="Market Technical Indicators">
          <div className="grid grid-cols-2 gap-4">
            <SubMetric
              label="1-Day Return"
              value={fmtPct(p.market_returns.return_1d)}
            />
            <SubMetric
              label="5-Day Return"
              value={fmtPct(p.market_returns.return_5d)}
            />
          </div>
        </Panel>
      </div>
      ...
      <Panel
        ...
        subtitle="Top matches from vector similarity search"
        className="mt-4"
      >
        {p.similar_events?.length > 0 ? (
          <div className="space-y-3">
            {p.similar_events.map((ev, i) => (
              <div
                key={i}
                className="flex items-center justify-between gap-4 py-2 border-b border-ink-600/60 last:border-0"
              >
                <div className="min-w-0">
                  <div className="text-sm text-paper truncate">{ev.title}</div>

                  <div className="text-xs text-paper-dim num mt-0.5">
                    {ev.date}
                  </div>
                </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-paper-dim">
            Derived from the model's expected move — informational only, not
            financial advice.
          </span>
        </div>
      </Panel>
...
function SubMetric({ label, value, delta }) {
  return (
    <div>
      ...
      {delta != null && (
        <div
          className={`num text-xs mt-0.5 ${delta >= 0 ? "text-signal-up" : "text-signal-down"}`}
        >
          {delta >= 0 ? "+" : ""}
          {delta.toFixed(2)}
        </div>
