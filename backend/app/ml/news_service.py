...
def get_aggregated_news_features(top_k: int = 3):
    articles = fetch_news_articles(num_articles=3)
    ...
    for article in articles:
        results, count = find_similar_events(
            article["text"],
            top_k=top_k
        )
    ...
    # Keep only the 3 most similar events overall
    ...
    aggregated = {
        k: float(np.mean([f[k] for f in all_features]))
        for k in keys
    }

    aggregated["news_count"] = int(
        sum(f["news_count"] for f in all_features)
    )

    aggregated["has_supply_chain_event"] = int(
        any(f["has_supply_chain_event"] for f in all_features)
    )
